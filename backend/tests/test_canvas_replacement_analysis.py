...
def test_normalise_replacement_objects_requires_numeric_shot_indices():
    ...
    assert objects == [{
        ...
        "actions": [
            ...
            {"shot_index": 2, "description": "托盘 出现在当前镜头中，保持原有位置、动作与遮挡关系。"},
            {"shot_index": 20, "description": "托盘 出现在当前镜头中，保持原有位置、动作与遮挡关系。"},
        ],
    }]
...
def test_per_shot_video_prompt_uses_the_external_template_and_numeric_shot_actions(tmp_path):
    ...
    service = CanvasReplacementTaskService(
        ...
        config=CanvasReplacementVideoConfig(
            api_key="", api_url="https://example.invalid/tasks", max_asset_bytes=1, ffmpeg_binary="ffmpeg"
        ),
    ...
    prompts = service.build_prompts(
        ...
        actions=[{"shot_index": 3, "description": "手持展示托盘正面。"}],
    )
    ...
    assert "手持展示托盘正面" in prompts[0]["prompt"]
