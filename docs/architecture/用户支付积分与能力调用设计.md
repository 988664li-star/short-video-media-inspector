...
| 用户主动取消 | 默认不退款；仅允许任务处于 `PENDING` 时取消，任务已开始执行后不提供用户取消 |
...
用户登录 Web
  ...
  → MySQL：创建任务 + 预扣积分（同一事务）
  → API：执行 FFmpeg / Whisper / 分镜 / AI 模型，或调用外部异步能力
...
```text
Web 轮询任务状态接口，或订阅 /api/v1/realtime
API 在任务完成后更新任务状态
通知服务 → WebSocket / SSE → 浏览器刷新页面状态
```

任务的最终结果始终从 MySQL/MinIO 查询；浏览器不以进程内任务状态作为事实来源。
...
9. 用户服务刷新可用能力规则/权益缓存；通知服务发送支付成功通知
```
...
7. 返回 `202 + job_id`；由 API 或外部能力服务执行任务
8. 执行完成后更新任务状态并保存结果
...
- 用户主动取消默认不退款；为避免争议，仅在任务尚未开始、状态为 `PENDING` 时允许取消。
- 任务进入 `RUNNING` 后用户不能取消；如需人工处理，由管理员通过补偿账本发放积分。
...
`orders`、`payment_transactions`、`analysis_jobs`、`credit_ledger` 必须保存关联业务 ID 和幂等键，方便防重、审计和补偿。
...
5. 根据任务耗时决定在 API 内执行，或对接供应商的异步任务接口；任务状态统一写入 MySQL。
